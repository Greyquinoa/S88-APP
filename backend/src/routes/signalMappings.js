...
router.get('/project/:projectId/signals', (req, res) => {
  try {
    const db        = getDb();
    const projectId = parseInt(req.params.projectId, 10);
    const importId  = latestHwImportId(db, projectId);
...
router.put('/project/:projectId/instance/:instanceName', (req, res) => {
  try {
    const db           = getDb();
    const projectId    = parseInt(req.params.projectId, 10);
    ...
    const save = db.transaction(() => {
      ...
      for (const m of clean) {
        ins.run(projectId, instanceName, m.blockName, m.varName, m.signalTag,
...
router.delete('/:id', (req, res) => {
  try {
    const db = getDb();
    db.prepare(`DELETE FROM signal_mappings WHERE id = ?`).run(parseInt(req.params.id, 10));
