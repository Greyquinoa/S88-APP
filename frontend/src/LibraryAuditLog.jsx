...
import { getLibraryAuditLog } from "./api.js";
...
function LibraryAuditLog({ projectId }) {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const result = await getLibraryAuditLog(projectId, {
        limit, offset,
  ...
  }, [projectId, offset, actionFilter, userFilter, fromFilter, toFilter]);

  useEffect(() => { load(); }, [load]);
  useEffect(() => { setOffset(0); }, [actionFilter, userFilter, fromFilter, toFilter]);
  ...
  const columnDefs = useMemo(() => [
    ...
    {
      headerName: 'Object', field: 'object_label',
      filter: 'agTextColumnFilter', floatingFilter: true, minWidth: 140, flex: 0.9,
      valueFormatter: p => p.value || '—',
      cellStyle: { fontSize: 12, fontFamily: 'var(--font-mono)' },
    },
    {
      headerName: 'Action', field: 'field_changes',
    ...
    <div>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 14, flexWrap: "wrap", alignItems: "center" }}>
        ...
        <input
          ...
          style={{
            ...
            color: "var(--color-text-primary)", width: 150
          }}
        />
        <input
      ...
      <div className="ig-root" style={{ width: '100%', background: '#FFFFFF' }}>
        <AgGridReact
          ...
          animateRows={false}
          loading={loading}
          overlayNoRowsTemplate="No audit entries match the selected filters."
        />
      </div>
...
export default LibraryAuditLog;
