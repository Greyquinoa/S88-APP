...
import {
  Btn, PanelHeading, Callout, StatTile, Tag,
  panelSx, panelHeaderSx, textInputSx, EmptyState, ROW_SELECTED_BG,
...
// Btn has no size prop — compact variants pass this through `style`.
const smBtnSx = { padding: '4px 10px', fontSize: 12 };

export default function StepReconciliation({ projectId }) {
  ...
  // A selection made before a filter change can leave hidden rows selected; bulk
  // actions would then hit rows the user can no longer see.
  ...
  const handleRowAccept = async (id) => {
    setError(null);
    try { await bulkAcceptDummyInstances([id], 'user'); await load(); }
    catch (e) { setError(e.message); }
  };

  const handleRowRevert = async (id) => {
    setError(null);
    try { await bulkRevertDummyInstances([id]); await load(); }
    catch (e) { setError(e.message); }
  };

  if (!projectId) {
    return <EmptyState>Select a project to reconcile its instances.</EmptyState>;
  }
      ...
      <div style={panelSx}>
        <div style={panelHeaderSx}>
          <PanelHeading
            ...
            subtitle="The IO list is the source of truth. Instances that exist only in unit type generation are dummies and are excluded from the XML export until accepted."
          />
        </div>
